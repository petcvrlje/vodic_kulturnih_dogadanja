...
class LoginViewController: UIViewController {
    ...
    @IBAction func loginButtonClick(_ sender: UIButton) {
        ...
        Alamofire.request(URL_USER_LOGIN, method: .post, parameters: params, encoding: JSONEncoding.default).responseJSON {
            ...
            //provjera statusCoda samo ako je 200 onda je login uspješan i kreira se token
            if (statusCode != 200) {
                let alertController = UIAlertController(title: "LOGIN UNSUCESSFUL", message: nil, preferredStyle: .alert)
